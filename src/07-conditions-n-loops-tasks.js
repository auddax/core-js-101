...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const rect2x = rect2.left + rect2.width;
  const rect2y = rect2.top + rect2.height;

  return (!(rect1x < rect2.left || rect2x < rect1.left)
    && !(rect1y < rect2.top || rect2y < rect1.top));
}
...
function isCreditCardNumber(ccn) {
  ...
  let index = arr.length % 2 === 0 ? 0 : 1;
  let sum = 0;
  for (let i = 0; i < arr.length; i += 1) {
    if (i === index) {
      index += 2;
      const prod = 2 * Number(arr[i]);
...
function getCommonDirectoryPath(pathes) {
  for (let j = 0; j < pathes[0].length; j += 1) {
    for (let i = 1; i < pathes.length; i += 1) {
      if (pathes[0].slice(0, j) !== pathes[i].slice(0, j)) {
        const sep = pathes[i].slice(0, j).lastIndexOf('/');
        return pathes[i].slice(0, j).slice(0, sep + 1);
      }
    }
  }
  return pathes[0];
